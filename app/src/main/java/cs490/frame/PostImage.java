...
public class PostImage extends AsyncTask<Post, Void, Boolean> {
    ...
    protected Boolean doInBackground(Post...params) {
        ...
        Post post = new Post();
        post = params[0];
        Log.i("postImage", "Post Lat: " + post.getLat() + " Lng: " + post.getLng() + " picture: lookin' good" + " user: " + post.getUser());

        ImageBean image = new ImageBean();
        image.setData(post.getPicture());

        try {
            MyBean response = myApiService.postImage(post.getUser(), post.getUserEmail(), post.getLat(), post.getLng(), image).execute();
